...
(function ()
{
    this.ForecastIO =
    {
        ...
        _init:
            function (settings)
            {
                ForecastIO._settings =
                    $.extend
                    (
                        { },
                        ForecastIO._defaultSettings,
                        settings || { }
                    );
        ...
        _onForecastReceived:
            function (forecast)
            {
                ...
                (
                    [
                        'ForecastIO._onForecastReceived(): received',
                ...
                ForecastIO._updateSunriseSunset(forecast);
                ForecastIO._updateMoonPhase(forecast);
                ForecastIO._updateHourlyForecast(forecast);
                ForecastIO._updateDailyForecast(forecast);
            },
            ...
            function (forecast)
            {
                ...
                $('.CURRENT_ICON')
                    ...
                    (
                        [
                            'CURRENT_ICON',
                            ForecastIO._getForecastIcon(current.icon)
                        ]
                        .join(' ')
                    );
        ...
        _updateSunriseSunset:
            function (forecast)
            {
                ...
                WxDashboard.formatTime(sunrise, $('.SUNRISE'));
                WxDashboard.formatTime(sunset, $('.SUNSET'));
            },
        ...
        _updateHourlyForecast:
            function (forecast)
            {
                ...
                ForecastIO._checkPrecipitation
                (
                    forecast.currently,
                    $('.TODAY_PRECIP')
                );
                ...
                for (var i = 0;
                         i < Math.min
                             (
                                hourly.data.length,
                                ForecastIO._settings.hourlyForecastLength
                             );
                         i += 2)
                {
                    ...
                    ForecastIO._addHourlyForecast(hourlyData, $clone);
                }
            },
            ...
            function (hourlyData, $target)
            {
                ...
                $target
                    .find('.HOURLY_ICON')
                        ...
                        .addClass
                        (
                             ForecastIO._getForecastIcon(hourlyData.icon)
                        )
                        .end()
                    .find('.HOURLY_TEMP')
                        .html
            ...
            function (forecast)
            {
                ...
                for (var i = 1;
                     i < Math.min(daily.data.length,
                                  1 + ForecastIO._settings.dailyForecastLength);
                     i++)
                {
                    ...
                    ForecastIO._addDailyForecast(dailyData, $clone);
                }
            },
        ...
        _addDailyForecast:
            function (dailyData, $target)
            {
                $target
                    .find('.DAILY_ICON')
                        .addClass(ForecastIO._getForecastIcon(dailyData.icon))
                        .end()
                    .find('.DAILY_TEXT')
                        .html
                        (
                            ForecastIO._getDailyForecastText(dailyData)
                        )
                        .end()
                    .find('.DAILY_TEMPS')
                        .html
        ...
        _getDailyForecastText:
            function (dailyData)
            {
                ...
                (
                    text,
                    [
                        WxDashboard.getWeekdayName(date),
                        ': ',
                        dailyData.summary,
                        ' '
                    ]
            ...
            function (f, props)
            {
                ...
                props =
                    ...
                    (
                        {
                            digits: 1,
                            temp: ForecastIO._settings.temp,
                            showTemp: true
                        },
                        props || { }
                    );
